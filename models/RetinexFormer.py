...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        print(f"illu_attn shape: {illu_attn.shape}")

        v = v * illu_attn  # Ensure v and illu_attn have matching dimensions
        q = q.transpose(-2, -1)
        k = k.transpose(-2, -1)
        v = v.transpose(-2, -1)
...
class RetinexFormerEncoder(RetinexFormer_Single_Stage):
    """
    继承自 RetinexFormer_Single_Stage，仅用于提取等分辨率特征，不执行最终图像映射。
    """
    def __init__(self, in_channels=3, n_feat=40, level=1, num_blocks=[1, 2, 2]):
        """
        初始化参数与原编码器配置对齐
        """
        super().__init__(
            in_channels=in_channels,
            out_channels=3,  # 仅需特征提取，输出通道数不影响
            n_feat=n_feat,
            level=level,
    ...
    def forward(self, img):
        # 1. 估计光照特征和光照图（与原逻辑一致）
        illu_fea, illu_map = self.estimator(img)
        x = img * illu_map + img  # 生成加权输入

        # 2. 特征投影与编码（去除 Denoiser 的最终映射，仅返回中间特征）
        fea = self.denoiser.embedding(x)  # 输入投影
        feats, illu_list = [], []

        # 编码器阶段：提取多级特征（与原逻辑一致）
        for (igab, downF, downI) in self.denoiser.encoder_layers:
            fea = igab(fea, illu_fea)
            feats.append(fea)
            illu_list.append(illu_fea)
            fea = downF(fea)
            illu_fea = downI(illu_fea)

        # 瓶颈层（与原逻辑一致）
        fea = self.denoiser.bottleneck(fea, illu_fea)

        # 解码器阶段：恢复特征分辨率（与输入一致）
        for (upF, fuse, igab) in self.denoiser.decoder_layers:
            fea = upF(fea)
            skip = feats.pop()
            fea = fuse(torch.cat([fea, skip], dim=1))
            illu_fea = illu_list.pop()
            fea = igab(fea, illu_fea)

        # 返回等分辨率特征图（形状与输入图像一致：[b, C_feat, h, w]）
        return fea
